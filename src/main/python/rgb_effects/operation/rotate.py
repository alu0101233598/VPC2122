  ...
  for n, _ in enumerate(transposed_data):
    for i in range(transposed_data.height):
      for j in range(transposed_data.width):
        transposed_data[n][i * transposed_data.width + j] = image_data[n][j * image_data.width + image_data.width - i - 1]
...
def rotate_180(image_data):
  ...
  for n, _ in enumerate(transposed_data):
    for i in range(transposed_data.height):
      for j in range(transposed_data.width):
        transposed_data[n][i * transposed_data.width + j] = image_data[n][(image_data.height - i - 1) * image_data.width + image_data.width - j - 1]
